...
use crate::{calculate_accrued_interest, state::*};
...
pub fn process_repay(ctx: Context<Repay>, amount: u64) -> Result<()> {
    ...
    bank.total_borrowed = calculate_accrued_interest(bank.total_borrowed, bank.interest_rate, user.last_updated_borrow)?;


     let value_per_share = bank.total_borrowed / bank.total_borrowed_shares;

    let user_value = borrowed_value as f64 * value_per_share as f64;
