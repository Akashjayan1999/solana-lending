...
pub fn process_deposit(ctx: Context<Deposit>, amount: u64) -> Result<()> {
    // Step 1: Transfer tokens from user to bank via CPI
    let transfer_cpi_accounts = TransferChecked {
        from: ctx.accounts.user_token_account.to_account_info(),
        mint: ctx.accounts.mint.to_account_info(),
        to: ctx.accounts.bank_token_account.to_account_info(),
    ...
    // Step 2: Calculate shares for the depositor
    let bank = &mut ctx.accounts.bank;
    ...
    if bank.total_deposits == 0 {
        // First deposit: 1:1 ratio of deposits to shares
        users_shares = amount;
    } else {
        // Subsequent deposits: proportional share calculation
        // deposit_ratio = new_deposit / existing_deposits
        // users_shares = existing_shares * deposit_ratio
    ...
    // Step 3: Update user's account based on which token is being deposited
    let user = &mut ctx.accounts.user_account;
    ...
    // Step 4: Update bank's total deposits and shares
    bank.total_deposits = bank.total_deposits
        .checked_add(amount)
        .ok_or(ErrorCode::MathOverflow)?;
    bank.total_deposit_shares = bank.total_deposit_shares
        .checked_add(users_shares)
        .ok_or(ErrorCode::MathOverflow)?;
