...
pub fn process_borrow(ctx: Context<Borrow>, amount: u64) -> Result<()> {
    ...
    // let users_shares = bank.total_borrowed_shares.checked_mul(borrow_ratio).unwrap();
     ...
     if bank.total_borrowed == 0 {
        ...
        let borrow_ratio = amount
         ...
         users_shares = bank.total_borrowed_shares
    ...
    bank.total_borrowed = bank.total_borrowed
        .checked_add(amount)
        .ok_or(ErrorCode::MathOverflow)?;
    bank.total_borrowed_shares = bank.total_borrowed_shares
...
pub fn calculate_accrued_interest(deposited: u64, interest_rate: f64, last_update: i64) -> Result<u64> {
